            ...
            switch (choice) {
                ...
                case 3 : {
                    ...
                    do {
                        ...
                        if (choice3 == 1) {
                            if (condition == 1) {
                                System.out.println("Add condition, separate by semicolon [filtered (e.g name, teamid);= or !=;filter");
                                ...
                                try {
                                    String filtered = temp[0];
                                    String[] filter = {temp[1], temp[2]};
                                    String table = null;

                                    if (join) {
                                        table = "Team";
                                        ArrayList<Users> filteredUser = uRep.find(filtered, filter, join, table, connect);
                                        Teams joinTeam;
                                    ...
                                    if (!join) {
                                        ArrayList<Users> filteredUser = uRep.find(filtered, filter, false, null, connect);
                            ...
                            if (condition == 1) {
                                System.out.println("Add condition, separate by semicolon [filtered;= or !=;filter");
                                String[] temp = scan.nextLine().split(";");
                                try {
                                    String filtered = temp[0];
                                    String[] filter = {temp[1], temp[2]};

                                    ArrayList<Teams> showTeam = tRep.find(filtered, filter, connect);
